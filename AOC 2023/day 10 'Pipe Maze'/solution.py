"""
You use the hang glider to ride the hot air from Desert Island all the way up to the floating metal island. This island is surprisingly cold and there definitely 
...
Map = Fn.getlines("C:/Users/timot/Documents/Programmation/Python/Advent of code 2023/day 10 'Pipe Maze'/input.txt")

Tiles = {'|' : [True, False, True, False], '-' : [False, True, False, True], 'L' : [True, True, False, False], 'J' : [True, False, False, True], 
        '7' : [False, False, True, True], 'F' : [False, True, True, False], '.' : [False, False, False, False]}
"""
| is a vertical pipe connecting north and south.
- is a horizontal pipe connecting east and west.
L is a 90-degree bend connecting north and east.
...
Translated to lists of format [North, East, South, West], 
"""
...
def previousDirection(ppoint, cpoint):
    px, py = ppoint
    cx, cy = cpoint
    diff = (cx-px, cy-py)
...
def nextPoint(map, path):
    cx, cy = path[-1]
    
    tile = Tiles[map[cx][cy]]
    for i in range(4):
        if i != previousDirection(path[-2], path[-1]) and tile[i]:
            nextDir = i
    
    return DirtoPoint((cx, cy), nextDir)

def canConnect(map, path, point):
    px, py = point
    if map[px][py] == 'S':
        return True
    else:
        nextTile = Tiles[map[px][py]]
        return nextTile[previousDirection(path[-1], (px, py))]

def findLoop(map, Start):
    for startDir in range(4):
        path = [Start, DirtoPoint(Start, startDir)]
        Point = nextPoint(map, path)
        isStart = False
        while (not isStart) and canConnect(map, path, Point):
            path.append(Point)
            Point = nextPoint(map, path)
            if Point == Start:
                isStart = True
...
Loop = findLoop(Map, findS(Map))
SOLUTION_p1 = len(Loop)//2
print(SOLUTION_p1)
...
The above loop encloses merely four tiles - the two pairs of . in the southwest and southeast (marked I below). The middle . tiles (marked O below) are not in 
...
In fact, there doesn't even need to be a full tile path to the outside for tiles to count as outside the loop - squeezing between pipes is also allowed! Here, 
...
Any tile that isn't part of the main loop can count as being enclosed by the loop. Here's another example with many bits of junk pipe lying around that aren't 
...
Mask = [[False]*len(Map[x]) for x in range(len(Map))]
