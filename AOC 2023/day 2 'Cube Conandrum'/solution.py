...
The Elf explains that you've arrived at Snow Island and apologizes for the lack of snow. He'll be happy to explain the situation, but it's 

a bit of a walk, so you have some time. They don't get many visitors up here; would you like to play a game in the meantime?


As you walk, the Elf shows you a small bag and some cubes which are either red, green, or blue. Each time you play this game, he will hide 

a secret number of cubes of each color in the bag, and your goal is to figure out information about the number of cubes.
...
(like the 11 in Game 11: ...) followed by a semicolon-separated list of subsets of cubes that were revealed from the bag (like 3 red, 
...
games_lines = Fn.getlines("day 2 'Cube Conandrum'/input.txt") # ["Game 1: 1 green, 1 blue, 1 red; ...", ...]

n = len(games_lines)
games = [""]*n
for i in range(n):
    games[i] = games_lines[i].split(": ")[1] # ["1 green, 1 blue, 1 red; ...", ...]
games_sets = [games[i].split("; ") for i in range(n)] # [["1 green, 1 blue, 1 red", ...], ...]

games_sets_nb_col = []
for i in range(n):
    sets = games_sets[i] # ["1 green, 1 blue, 1 red", ...]
    game_sets_nb_col = []
    for j in range(len(sets)):
        set = sets[j].split(", ") # ["1 green", "1 blue", "1 red"]
        set_nb_col = [0, 0, 0] # Nombres de cubes dans chaque couleur rangées dans l'ordre RGB : [1, 1, 1] ici
        for k in range(len(set)):
            nb_col = set[k].split(" ") # ["1", "green"]
            match nb_col[1]:
                case "red":
                    set_nb_col[0] += int(nb_col[0])
                case "green":
                    set_nb_col[1] += int(nb_col[0])
                case "blue":
                    set_nb_col[2] += int(nb_col[0])
        game_sets_nb_col.append(set_nb_col) # [[1, 1, 1], ...]
    games_sets_nb_col.append(game_sets_nb_col) # [[[1, 1, 1], ...], ...]
...
for i in range(n):
    validgame = True
    for set in games_sets_nb_col[i]:
        if set[0] > nred or set[1] > ngreen or set[2] > nblue:
            validgame = False
            break
    if validgame:
        IDvalidgames.append(i+1)

SOLUTION_p1 = sum(IDvalidgames)
print(SOLUTION_p1)


"""
The Elf says they've stopped producing snow because they aren't getting any water! He isn't sure why the water stopped; however, he can show you 
...
setpows = []
for set in minsets:
    setpows.append(setpow(set))

SOLUTION_p2 = sum(setpows)
print(SOLUTION_p2)