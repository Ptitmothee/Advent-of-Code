...
Lines = Fn.getlines("C:/Users/timot/Documents/Programmation/Python/Advent of code 2023/day 9 'Mirage Maintenance'/input.txt")
n = len(Lines)
Histories = []
for i in range(n):
    line_list = Lines[i].split(" ")
    Histories.append(Fn.list_StrToInt(line_list))
...
For each history, repeat the process of finding differences until the sequence of differences is entirely zero. Then, rather than adding a zero to the end and 

filling in the next values of each previous sequence, you should instead add a zero to the beginning of your sequence of zeroes, then fill in new first values 
...
Doing this for the remaining example data above results in previous values of -3 for the first history and 0 for the second history. Adding all three new 
