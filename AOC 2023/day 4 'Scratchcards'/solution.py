...
off. Picking one up, it looks like each card has two lists of numbers separated by a vertical bar (|): a list of winning numbers and then a list 
...
In the above example, card 1 has five winning numbers (41, 48, 83, 86, and 17) and eight numbers you have (83, 86, 6, 31, 17, 9, 48, and 53). Of 

the numbers you have, four of them (48, 83, 17, and 86) are winning numbers! That means card 1 is worth 8 points (1 for the first match, then 
...
cards_lines = Fn.getlines("day 4 'Scratchcards'/input.txt")
n = len(cards_lines)

cards = [cards_lines[i].split(": ")[1].split(" | ") for i in range(n)]
# [["27 61 49 69 58 44  2 29 39 10", "97 96 49 78 26 58 27 77 69  9 39 88 53 10  2 29 61 62 48 87 18 44 74 34 11"], ...]

cards_nb = []
for i in range(n):
    card = cards[i]

    win_numbers_str = card[0].split()
    numbers_str = card[1].split()

    win_numbers = [int(nb) for nb in win_numbers_str]
    numbers = [int(nb) for nb in numbers_str]

    cards_nb.append([win_numbers, numbers])
...
cards_scores = [cardscore(card) for card in cards_nb]

SOLUTION_p1 = sum(cards_scores)
print(SOLUTION_p1)
...
Copies of scratchcards are scored like normal scratchcards and have the same card number as the card they copied. So, if you win a copy of card 

10 and it has 5 matching numbers, it would then win a copy of the same cards that the original card 10 won: cards 11, 12, 13, 14, and 15. This 

process repeats until none of the copies cause you to win any more cards. (Cards will never make you copy a card past the end of the table.)
...
instances of card 4, 14 instances of card 5, and 1 instance of card 6. In total, this example pile of scratchcards causes you to ultimately 
...
for i in range(n):
    ...
    for j in range(count):
        ...
        if k >= n:
            ...
            nb_of_cards[k] += nb_of_cards[i]

SOLUTION_p2 = sum(nb_of_cards)
print(SOLUTION_p2)